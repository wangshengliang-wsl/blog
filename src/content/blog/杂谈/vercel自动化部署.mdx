...
| Secret 名称         | 说明             | 获取方式                                      |
| ------------------- | ---------------- | --------------------------------------------- |
| `VERCEL_TOKEN`      | Vercel API Token | 从 Vercel Account Settings 创建               |
| `VERCEL_ORG_ID`     | Vercel 组织 ID   | 从 `.vercel/project.json` 或 Vercel Dashboard |
| `VERCEL_PROJECT_ID` | Vercel 项目 ID   | 从 `.vercel/project.json` 或 Vercel Dashboard |
...
其实在vercel主页通过项目导入的方式，push代码后也会自动部署，但是可能会部署失败（缺乏前置校验逻辑），配置 GitHub Actions 能够前置捕获错误，提前发现问题，确保部署成功。
